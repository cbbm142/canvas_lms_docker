...
RUN set -eux; \
    git clone --single-branch --branch stable https://github.com/instructure/canvas-lms.git .
...
RUN yarn install --ignore-optional --pure-lockfile --network-concurrency 1
...
COPY --from=builder --chown=docker:docker ${APP_HOME} .
COPY --from=builder --chown=docker:docker ${GEM_HOME} ${GEM_HOME}
